...
- (void)initAppSetting{
    ...
    //云之讯token
    [[NSNotificationCenter defaultCenter ] addObserver:self
                                              selector:@selector(receiveUCSData:)
                                                  name:kNotificationUCSTokenFinish
                                                object:nil];
        ...
        if ([expirationDateStr length]) {
            ...
            if ([expirationDate timeIntervalSinceDate:[NSDate date]]<15*24*60*60)
            {
                if (![[ZdywUtils getLocalStringDataValue:kExpirationDateTip] isEqualToString:[dateFormatter stringFromDate:[NSDate date]]])
                {
                    ...
                    NSString *msgStr = [NSString stringWithFormat:@"你当前使用的企业版顶讯电话%@就要过期了，赶紧更新最新的版本，谢谢支持",dateStr];
...
//云之讯连接token
-(void)receiveUCSData:(NSNotification *)notification{
    
    NSDictionary *dic = [notification userInfo];
    NSLog(@"取到云之讯token=%@",dic);
     //云之讯token
    if ([dic.allKeys containsObject:@"token"]&&[dic objectForKey:@"token"]&&[[dic objectForKey:@"token"] length]>0) {
        NSString *token = [ZdywUtils getLocalStringDataValue:kUCSTokenId];
        if (![token isEqualToString:[dic objectForKey:@"token"]]) {
            [ZdywUtils setLocalDataString:[dic objectForKey:@"token"] key:kUCSTokenId];
            [self.ucsFuncEngine  connect:[dic objectForKey:@"token"]];
        }else{
            if (![self.ucsFuncEngine isConnected]) {//表示未连接
                [self.ucsFuncEngine  connect:[dic objectForKey:@"token"]];
            }
        }
    }
    /***
    //云之讯clientNumber
    if ([dic.allKeys containsObject:@"clientNumber"]&&[dic objectForKey:@"clientNumber"]&&[[dic objectForKey:@"clientNumber"] length]>0) {
        [ZdywUtils setLocalDataString:[dic objectForKey:@"clientNumber"] key:kUCSClientNumberId];
    }
    //通知token请求完成
    [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationUCSTokenFinished object:nil userInfo:dic];
    ****/
