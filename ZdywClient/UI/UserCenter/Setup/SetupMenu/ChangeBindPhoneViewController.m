...
- (void)countCaptachaTime{
    if (_captchaTime > 0) {
        ...
        [_sendVericaBtn setTitle:getCaptchaTimeStr forState:UIControlStateDisabled];
...
- (void)captchaAction{
    ...
    //获取passwd(密码要md5)
    NSString *strPwd = _oldPwdTextField.text;
...
- (void)textFieldDidBeginEditing:(UITextField *)textField{
    if ((textField == _phoneTextField)||(textField == _verificaTextField)) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue]>=7.0) {
            [self.mianScrollView setContentOffset:CGPointMake(0, 80)];
        }else {
            [self.mianScrollView setContentOffset:CGPointMake(0, 120)];
        }
    } else {
        if ([[[UIDevice currentDevice] systemVersion] floatValue]>=7.0) {
        [self.mianScrollView setContentOffset:CGPointMake(0, -64)];
