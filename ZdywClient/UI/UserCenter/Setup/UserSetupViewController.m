...
- (void)buildMoreDataModel
{
    ...
    rowNode = [[MoreRowNode alloc] init];
    rowNode.tag =Setup_ShowPhone;
    rowNode.mainTitle = @"去电显号";
    [sectionNode.child addObject:rowNode];
...
- (void)swithShowPhonAction:(UISwitch *)swith{
    int soundOpen = [[ZdywUtils getLocalIdDataValue:kCallShowPhoneFlag] intValue];
    if (soundOpen == 0)
    {
        soundOpen = 1;
    } else {
    ...
    [ZdywUtils setLocalIdDataValue:[NSString stringWithFormat:@"%d",soundOpen] key:kCallShowPhoneFlag];
}
...
- (void)logout{
    ...
    [ZdywUtils setLocalDataString:@"" key:kUCSTokenId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (node.tag) {
        ...
        case Setup_ShowPhone:{
            ...
            [swithBtn addTarget:self action:@selector(swithShowPhonAction:) forControlEvents:UIControlEventValueChanged];
