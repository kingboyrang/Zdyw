...
- (void)showCallView
{
    ...
    self.isCalling = YES;
    _directCallView = [[DirectCallViewController alloc] initWithNibName:NSStringFromClass([DirectCallViewController class]) bundle:nil];
    _directCallView.callInfoNode=self.myCallInfoNode;
    _navigation = [[UINavigationController alloc] initWithRootViewController:_directCallView];
    [[ZdywAppDelegate appDelegate].window addSubview:_navigation.view];
    
    //[_directCallView startCall:self.myCallInfoNode];
    /***
    self.isCalling = YES;
    UCSDirecViewController  *second= [[UCSDirecViewController alloc] init];
    _navigation = [[UINavigationController alloc] initWithRootViewController:second];
    [[ZdywAppDelegate appDelegate].window addSubview:_navigation.view];
     ***/
}
...
- (void)showCallBackView{
    ...
    _callBackView.myCallInfoNode=self.myCallInfoNode;
...
- (void)initiatingCall:(CallInfoNode *)callInfoNode
{
    ...
    {
        NSString *strMessage = @"您当前网络不支持顶讯拨打，请检查网络";
...
- (void)showTipview:(NSString*)resultStr{
    ...
    [tipView.backButton setTitleColor:[UIColor colorWithRed:25.0/255 green:146.0/255 blue:215.0/255 alpha:1.0] forState:UIControlStateNormal];
